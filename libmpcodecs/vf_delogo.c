...
static int put_image(struct vf_instance *vf, mp_image_t *mpi, double pts){
	...
	if (vf->priv->endpos == 0 || (mpi->number >= vf->priv->startpos && mpi->number < vf->priv->endpos)) {
...
#endif
    { "start", ST_OFF(startpos), CONF_TYPE_INT, 0, 0, 0, NULL },
    { "end", ST_OFF(endpos), CONF_TYPE_INT, 0, 0, 0, NULL },
